...
- 소스 주소: https://github.com/alska14/ai-pair-log (공개 화면은 저장소 루트의 `index.html` 단일 파일. React+Vite로 실험했던 버전은 `web/` 폴더에 이력으로만 남아 있고 배포되지 않음)
...
- AI 제안을 따르지 않은 일: 회사 업무 저장소 내용을 마스킹해서 근거 자료로 복사하자는 제안을 거절했다 — 이름을 가려도 내부 설계·프로세스 자체가 기밀유지 대상일 수 있어 위험하다고 판단했다. 대신 이 페이지의 실제 제작 과정(`ai-pair-log`)을 근거로 썼다. 또한 AI가 만든 React+Vite 버전이 마음에 들지 않아 원래의 단일 HTML 디자인으로 직접 되돌리라고 지시했다.
